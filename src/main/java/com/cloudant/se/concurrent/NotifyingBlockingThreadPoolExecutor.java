 ...
 *
 * Two functionalities had been added to this subclass. 1) The execute method of the ThreadPoolExecutor will block in case the queue is full and only unblock when the queue is dequeued - that is a
 * task that is currently in the queue is removed and handled by the ThreadPoolExecutor. 2) Client code can await for the event of all tasks being run to conclusion. Client code which actively chose
 * to wait for this occurrence should call await on the instance of his ThreadPoolExecutor. This differs from awaitTermination as it does not require any call to shutdown.
 *
 * Code example:
 *
 ...
 *
 * The example above shows how 5000 tasks are run within 5 threads. The line with 'System.out.println("Done!");' will not run until such a time when all the tasks given to the thread pool have
 * concluded. their run.
 *
 * This subclass of ThreadPoolExecutor also takes away the max threads capabilities of the ThreadPoolExecutor superclass and internally sets the amount of maximum threads to be the size of the core
 * threads. This is done since threads over the core size and under the max are instantiated only once the queue is full, but the NotifyingBlockingThreadPoolExecutor will block once the queue is full.
 *
 *
 * As seen <a href="http://today.java.net/pub/a/today/2008/10/23/creating-a-notifying-blocking-thread-pool-executor.html"> here</a>.
 *
...
public class NotifyingBlockingThreadPoolExecutor extends ThreadPoolExecutor {
    ...
    /**
     * This is the Synchronizer instance that is used in order to notify all interested code of when all the tasks that have been submitted to the execute() method have run to conclusion. This
     * notification can occur a numerous amount of times. It is all up to the client code. Whenever the ThreadPoolExecutor concludes to run all the tasks the Synchronizer object will be notified and
     * will in turn notify the code which is waiting on it.
     */
    ...
    /**
     * This constructor is used in order to maintain the first functionality specified above. It does so by using an ArrayBlockingQueue and the BlockThenRunPolicy that is defined in this class. This
     * constructor allows to give a timeout for the wait on new task insertion and to react upon such a timeout if occurs.
     *
     * @param poolSize
     ...
     */
    public NotifyingBlockingThreadPoolExecutor(int poolSize, int queueSize, long keepAliveTime, TimeUnit keepAliveTimeUnit, long maxBlockingTime, TimeUnit maxBlockingTimeUnit,
        ...
        super(poolSize, // Core size
                ...
                threadFactory != null ? threadFactory : Executors.defaultThreadFactory(), // use default thread factory if they did not give us one
    ...
    /**
     * This constructor is used in order to maintain the first functionality specified above. It does so by using an ArrayBlockingQueue and the BlockThenRunPolicy that is defined in this class. Using
     * this constructor, waiting time on new task insertion is unlimited.
     *
     * @param poolSize
     *            is the amount of threads that this pool may have alive at any given time.
     * @param queueSize
     ...
     *            the factory to use when the executor creates a new thread - defaults to Executors.defaultThreadFactory if null
     */
    public NotifyingBlockingThreadPoolExecutor(int poolSize, int queueSize, long keepAliveTime, TimeUnit unit, ThreadFactory threadFactory) {
        super(poolSize, // Core size
                poolSize, // Max size
                keepAliveTime,
                unit,
    ...
    /**
     * A blocking wait for this ThreadPool to be in idle state, which means that there are no more tasks in the Queue or currently executed by one of the threads. BE AWARE that this method may get out
     * from blocking state when a task is currently sent to the ThreadPool not from this thread context. Thus it is not safe to call this method in case there are several threads feeding the TreadPool
     * with tasks (calling execute). The safe way to call this method is from the thread that is calling execute and when there is only one such thread. Note that this method differs from
     * awaitTemination, as it can be called without shutting down the ThreadPoolExecuter.
     *
    ...
    /**
     * A blocking wait for this ThreadPool to be in idle state or a certain timeout to elapse. Works the same as the await() method, except for adding the timeout condition.
     *
    ...
    /**
     * After calling super's implementation of this method, the amount of tasks which are currently in process is decremented. Finally, if the amount of tasks currently running is zero the
     * synchronizer's signallAll() method is invoked, thus anyone awaiting on this instance of ThreadPoolExecutor is released.
     *
    ...
    /**
     * This Policy class enforces the blocking feature of the NotifyingBlockingThreadPoolExecutor. It does so by invoking the BlockingQueue's put method (instead of the offer method that is used by
    ...
    private static class BlockThenRunPolicy implements RejectedExecutionHandler {
        ...
        /**
         * When this method is invoked by the ThreadPoolExecutor's reject method it simply asks for the Executor's Queue and calls on its put method which will Block (at least for the
         * ArrayBlockingQueue).
         *
    ...
    /**
     * This inner class serves to notify all interested parties that the ThreadPoolExecutor has finished running all the tasks given to its execute method.
     */
